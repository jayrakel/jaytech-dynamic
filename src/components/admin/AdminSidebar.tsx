...
export default function AdminSidebar({ user }: { user: { name?: string; email?: string } }) {
  ...
  // Close the mobile menu automatically when you click a link
  useEffect(() => {
    setIsOpen(false);
  }, [path]);
  ...
  return (
    <>
      ...
      `}>
        ...
        {/* Desktop Branding (Hidden on mobile top bar) */}
        <div className="p-5 border-b border-slate-800 hidden md:block">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center font-heading font-bold text-white text-xs" style={{background:"linear-gradient(135deg,#14B8A6,#3B82F6)"}}>JT</div>
            <div>
              <div className="font-heading font-bold text-xs leading-tight text-white">Jay TechWave</div>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
          {NAV.map(n => (
            <Link 
              key={n.href} 
              href={n.href} 
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive(n.href) ? "bg-teal-400/12 text-teal-400 border-l-2 border-teal-400 pl-2.5" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
            >
